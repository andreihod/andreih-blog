...
window.show_all();
```
The `connect_draw` clojure receives the cairo's [Context](https://gtk-rs.org/docs/cairo/struct.Context.html) as parameter (`cr`), that's the variable we'll use to draw the lines and labels, it's pretty much just like any other canvas. So let's set the background, the font and line width.
